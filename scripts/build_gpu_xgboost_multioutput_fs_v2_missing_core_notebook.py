...
ROOT = project_root()
NOTEBOOK_NAME = "data-fusion-2026-gpu-xgboost-multioutput-fs-v2-missing-core.ipynb"
NOTEBOOK_PATH = ROOT / "output" / "jupyter-notebook" / NOTEBOOK_NAME
PUSH_DIR = ROOT / "output" / "kaggle-push" / "xgboost-multioutput-fs-v2-missing-core"
KERNEL_METADATA_PATH = PUSH_DIR / "kernel-metadata.json"
NOTEBOOK_REVISION = "xgboost_multioutput_fs_v2_missing_core_rev1_20260321"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            dedent(
                """
                # Experiment: Data Fusion 2026 GPU XGBoost Multi-Output FS v2 Missing Core

                Objective:
                - Add a third backbone with a different tree bias from `CatBoost` and `TabM`.
                - Re-run the proven fast `XGBoost` multi-output pipeline on the new `fs_v2_missing_core` dataset.
        ...
        code_cell(
            dedent(
                ...
                ARTIFACT_DIR = Path("artifacts") / f"gpu_xgboost_multioutput_fs_v2_missing_core_{RESOLVED_MODE}"
        ...
        code_cell(
            dedent(
                ...
                def find_compact_feature_dir() -> Path:
                    direct_candidates = [
                        Path.cwd() / "artifacts" / "feature_selection_v2_missing_core",
                        Path.cwd() / "output" / "kaggle-datasets" / "task-artifacts" / "feature_selection_v2_missing_core",
                        Path("/kaggle/input/data-fusion-2026-2-task-artifacts/feature_selection_v2_missing_core"),
                        Path("/kaggle/input/data-fusion-2026-2-task-artifacts"),
                        Path.cwd(),
                    ]
                    required = [
                        "train_compact_features_v2_missing_core.parquet",
                        "test_compact_features_v2_missing_core.parquet",
                        "column_groups.json",
                    ]
                    for candidate in direct_candidates:
                        if all((candidate / name).exists() for name in required):
                    ...
                    kaggle_input = Path("/kaggle/input")
                    if kaggle_input.exists():
                        for match in kaggle_input.rglob("train_compact_features_v2_missing_core.parquet"):
                            candidate = match.parent
                            if all((candidate / name).exists() for name in required):
                                return candidate
                    raise FileNotFoundError("Could not locate the fs_v2_missing_core directory.")
        ...
        code_cell(
            dedent(
                ...
                train_features = pd.read_parquet(compact_feature_dir / "train_compact_features_v2_missing_core.parquet")
                test_features = pd.read_parquet(compact_feature_dir / "test_compact_features_v2_missing_core.parquet")
    ...
    kernel_metadata = {
        "id": "chesnikovleonid/data-fusion-2026-task-2-xgb-multiout-fs-v2-core",
        "title": "Data Fusion 2026 task 2 xgb multiout fs v2 core",
