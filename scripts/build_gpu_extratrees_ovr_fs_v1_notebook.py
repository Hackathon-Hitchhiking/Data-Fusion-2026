...
NOTEBOOK_NAME = "data-fusion-2026-extratrees-ovr-fs-v1.ipynb"
NOTEBOOK_PATH = ROOT / "output" / "jupyter-notebook" / NOTEBOOK_NAME
PUSH_DIR = ROOT / "output" / "kaggle-push" / "extratrees-ovr-fs-v1"
KERNEL_METADATA_PATH = PUSH_DIR / "kernel-metadata.json"
NOTEBOOK_REVISION = "extratrees_ovr_fs_v1_rev1_20260322"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            dedent(
                """
                # Experiment: Data Fusion 2026 ExtraTrees OvR FS v1

                Objective:
                - Test a very different donor bias for blending.
                - Use the lighter `fs_v1` feature space to keep the 41-model run practical.
                - Produce exact-holdout donor predictions for ensemble analysis.

                Note:
                - `ExtraTreesClassifier` is CPU-only.
                - Numerics are median-imputed, categoricals are ordinal-coded.
                """
            ).strip()
            + "\n",
            1,
        ...
        code_cell(
            dedent(
                ...
                ARTIFACT_DIR = PROJECT_ROOT / "artifacts" / f"extratrees_ovr_fs_v1_{RESOLVED_MODE}"
        ...
        code_cell(
            dedent(
                ...
                def find_feature_artifact_dir() -> Path:
                    ...
                    if kaggle_input.exists():
                        for match in kaggle_input.rglob("train_compact_features_v1.parquet"):
                            ...
                            if all((candidate / name).exists() for name in required):
                                return candidate
                    raise FileNotFoundError("Could not locate the feature artifact directory.")
                ...
                def prepare_et_frames(
                    ...
                    medians = prepared[0][num_cols].apply(pd.to_numeric, errors="coerce").median().astype("float32")
                    for col in cat_cols:
                    ...
                    for frame in prepared:
                        for col in num_cols:
                            frame[col] = pd.to_numeric(frame[col], errors="coerce").fillna(float(medians[col])).astype("float32")
        ...
        code_cell(
            dedent(
                ...
                train_features = pd.read_parquet(feature_artifact_dir / "train_compact_features_v1.parquet")
                test_features = pd.read_parquet(feature_artifact_dir / "test_compact_features_v1.parquet")
                ...
                train_x, valid_x, test_x = prepare_et_frames([train_part, valid_part, test_features], feature_cols, cat_cols)
                train_y = train_part[target_cols].astype("int8")
                valid_y = valid_part[target_cols].astype("int8")
                full_train_x, full_test_x = prepare_et_frames([train_features, test_features], feature_cols, cat_cols)
                full_train_y = target.astype("int8")

                print(
                    {
                        "base_data_dir": str(base_data_dir),
                        "feature_data_dir": str(feature_artifact_dir),
                ...
                print({"stage": "extratrees_ovr_start", "params": params}, flush=True)
                ...
                for idx, target_name in enumerate(target_cols, start=1):
                    ...
                    print({"stage": "extratrees_target_start", "target": target_name, "index": idx, "total": len(target_cols)}, flush=True)
                    ...
                    print({"stage": "extratrees_target_done", "target": target_name, "oof_auc": float(auc)}, flush=True)
                    if CFG["train_final_model"]:
                        final_clf = ExtraTreesClassifier(**params)
    ...
    kernel_metadata = {
        "id": "chesnikovleonid/data-fusion-2026-task-2-extratrees-ovr-fs-v1",
        "title": "Data Fusion 2026 task 2 extratrees ovr fs v1",
