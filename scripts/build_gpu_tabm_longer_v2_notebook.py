...
NOTEBOOK_PATH = ROOT / "output/jupyter-notebook/data-fusion-2026-gpu-tabm-multitask-longer-v2.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            dedent(
                """
                # Experiment: Data Fusion 2026 GPU TabM MultiTask Longer V2

                Objective:
                - Push the strong `TabM` backbone further after the first successful Kaggle run.
                - Test whether a longer schedule plus LR decay can beat the first `TabM` result.

                Design:
                - Multi-label training over all `41` targets with `BCEWithLogitsLoss`.
                - Longer training horizon with validation-driven LR decay.
                - Same running-artifact safety as the previous notebook.
                """
            ).strip()
            + "\n"
        ),
        ...
        code_cell(
            (
                ...
                + dedent(
                    ...
                    ARTIFACT_DIR = Path("artifacts") / f"gpu_tabm_longer_v2_{RESOLVED_MODE}"
        ...
        md_cell(
            dedent(
                ...
                - Compare this run against the first successful `TabM` notebook.
                - If it wins, the next public tests should be `pure TabM v2` and `TabM v2 + CatBoost`.
                """
            ).strip()
            + "\n"
        ),
