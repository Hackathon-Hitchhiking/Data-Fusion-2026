...
ROOT = project_root()
NOTEBOOK_NAME = "data-fusion-2026-gpu-xgboost-multioutput-fs-v1.ipynb"
NOTEBOOK_PATH = ROOT / "output" / "jupyter-notebook" / NOTEBOOK_NAME
PUSH_DIR = ROOT / "output" / "kaggle-push" / "xgboost-multioutput-fs-v1"
KERNEL_METADATA_PATH = PUSH_DIR / "kernel-metadata.json"
NOTEBOOK_REVISION = "xgboost_multioutput_fs_v1_rev1_20260321"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            dedent(
                """
                # Experiment: Data Fusion 2026 GPU XGBoost Multi-Output FS v1

                Objective:
                - Add a third backbone with a different tree bias from `CatBoost` and `TabM`.
                - Use the strongest compact `feature_selection_v1` dataset and select the final iteration count by external holdout `macro ROC-AUC`.
                ...
                - External checkpoint selection on the same holdout policy as the main `TabM`.
                """
            ).strip()
            + "\n",
            1,
        ...
        code_cell(
            dedent(
                ...
                ARTIFACT_DIR = Path("artifacts") / f"gpu_xgboost_multioutput_fs_v1_{RESOLVED_MODE}"
    ...
    kernel_metadata = {
        "id": "chesnikovleonid/data-fusion-2026-task-2-xgboost-multioutput-fs-v1",
        "title": "Data Fusion 2026 task 2 xgboost multioutput fs v1",
