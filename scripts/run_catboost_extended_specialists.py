...
DEFAULT_OUT_DIR = ROOT / "artifacts" / "catboost_extended_specialists"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run extended CatBoost specialists on hardest targets")
    p.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    p.add_argument("--hardest-count", type=int, default=12)
    p.add_argument("--boosting-type", choices=["Plain", "Ordered"], default="Ordered")
    p.add_argument("--baseline-mode", choices=["gandalf", "g95s05", "g90s10"], default="g95s05")
    ...
    p.add_argument("--weight-file", type=Path, default=None)
...
def load_train_weights(train_ids: pd.DataFrame, weight_file: Path | None, weight_column: str) -> np.ndarray:
    if weight_file is None:
        return np.ones(len(train_ids), dtype=np.float32)
    weight_df = pd.read_parquet(weight_file)[["customer_id", weight_column]].rename(columns={weight_column: "sample_weight"})
    merged = train_ids.merge(weight_df, on="customer_id", how="left")
    sample_weight = merged["sample_weight"].fillna(1.0).to_numpy(dtype=np.float32)
...
def main() -> None:
    ...
    train_weights = load_train_weights(train_df[["customer_id"]], args.weight_file, args.weight_column)
    ...
    for col in cat_cols:
        ...
        val_X[col] = val_X[col].fillna(-1).astype("int32")
    ...
    print(
        {"stage": "catboost_specialists_start", "target_count": len(hardest_targets), "feature_count": len(model_feature_cols)},
        flush=True,
    )
    ...
    for idx, target_name in enumerate(hardest_targets, start=1):
        ...
        print(
            {"stage": "catboost_target_start", "target": target_name, "index": idx, "positive_rate_train": float(y_train.mean())},
            flush=True,
        )

        train_pool = Pool(train_X, y_train, cat_features=cat_feature_indices, weight=train_weights)
        ...
        val_pred = model.predict_proba(val_pool)[:, 1].astype("float32")
        ...
        per_target_rows.append(
            {
                ...
                "delta_auc": float(cat_auc - baseline_auc),
                "best_iteration": int(model.get_best_iteration()),
                "boosting_type": args.boosting_type,
            }
        )
        print(
            {"stage": "catboost_target_done", "target": target_name, "catboost_auc": float(cat_auc), "delta_auc": float(cat_auc - baseline_auc)},
            flush=True,
        )
        ...
        del model, train_pool, val_pool, y_train, y_val, val_pred
        gc.collect()
