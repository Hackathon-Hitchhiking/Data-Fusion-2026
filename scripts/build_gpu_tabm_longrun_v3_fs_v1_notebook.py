...
ROOT = project_root()
FINAL_FEATURE_SET_FILE = ROOT / "artifacts" / "feature_selection_v1" / "final_feature_set.json"
...
def main() -> None:
    final_feature_set = json.loads(FINAL_FEATURE_SET_FILE.read_text())
    selected_extra_literal = json.dumps(final_feature_set.get("extra_features", []), ensure_ascii=True, indent=4)
    bootstrap_lines = [
        ...
        "import sys",
        "",
        'NOTEBOOK_REVISION = "tabm_longrun_v3_fs_v1_rev3_20260320"',
        "",
        "REQUIRED_PACKAGES = [",
        '    ("tabm", "tabm"),',
    ...
    cells = [
        md_cell(
            dedent(
                """
                # Experiment: Data Fusion 2026 GPU TabM LongRun V3 FeatureSet v1

                Objective:
                - Push the confirmed `TabM` backbone on the compact feature-selection-v1 dataset.
                - Keep the run production-safe, stage-logged, and recoverable on Kaggle.

                Design:
                - Multi-label training over all `41` targets with `BCEWithLogitsLoss`.
                - Compact feature block built from `feature_selection_v1` instead of raw `top100` extra merge.
                - `PiecewiseLinearEmbeddings(version="B")` for numerical features.
                - Robust numeric preprocessing with median/IQR scaling and smooth clipping.
                - Longer schedule, LR decay, top-k checkpoint averaging, and two-seed ensembling.
                """
            ).strip()
            + "\n"
        ),
        ...
        code_cell(
            (
                ...
                + dedent(
                    ...
                    ARTIFACT_DIR = Path("artifacts") / f"gpu_tabm_longrun_v3_fs_v1_{RESOLVED_MODE}"
                ...
                def make_num_embeddings(train_x_num: np.ndarray, cfg: dict[str, object], seed: int):
                    ...
                    if num_embedding_type == "piecewise":
                        ...
                        if not np.all(np.isfinite(bin_ptp) & (bin_ptp > 0)):
                            # Rare-but-real case: a feature varies on the full train set but becomes
                            # constant inside the random sample used for compute_bins. In that case,
                            # fall back to the full matrix instead of crashing in the final stage.
                            bin_source = train_x_num
        ...
        code_cell(
            dedent(
                ...
                FEATURE_DATA_DIR = find_compact_feature_dir()
                DEVICE = pick_device()
